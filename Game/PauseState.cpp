...
//-----------------------------------------------------------------------------
PauseState::PauseState(std::shared_ptr<GameController> game)
{
	this->m_game = game;
	...
	m_logo.setTexture(Resources::get_instance().getGeneralTexture(MAIN_LOGO_T));
	...
	m_exitButton.setPosition(m_resumeButton.getPosition() + sf::Vector2f{ 0.f, 120.f });
