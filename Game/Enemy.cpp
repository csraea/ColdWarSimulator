...
Enemy::Enemy()
{
	...
	m_teleport_sprite.setTexture(Resources::get_instance().getGeneralTexture(TEXTURES::TELEPORTATION_T));
	m_teleport_sprite.setOrigin(m_teleport_sprite.getGlobalBounds().width / 2, m_teleport_sprite.getGlobalBounds().height / 2);

	m_explosion.setTexture(Resources::get_instance().getGeneralTexture(DEATH_T));
